...
const getSimplifiedType = (schema: ZodTypeAny) => {
	if (schema instanceof ZodObject) {
		...
		const baseSchema = (schema as any)?.unwrap?.() ?? schema;
		return getSimplifiedType(baseSchema);
	}
...
export class N8nTool extends DynamicStructuredTool<ZodObjectAny> {
	...
	asDynamicTool(): DynamicTool {
		...
		const baseSchema = (schema as any)?.unwrap?.() ?? schema;
		const parser = new StructuredOutputParser(baseSchema);
		...
		const wrappedFunc = async function (query: string) {
			...
			try {
				parsedQuery = (await parser.parse(query)) as Record<string, unknown>;
			} catch (e) {
