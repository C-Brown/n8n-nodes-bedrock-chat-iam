...
export function getSessionId(
  ...
  // new: allow callers to override the expression we evaluate when autoSelect is used
  exprParamKey = 'sessionIdExpression', // e.g. defaults to `{{ $json.sessionId }}`
) {
  let sessionId = '';
  const selectorType = ctx.getNodeParameter(selectorKey, itemIndex) as string;

  if (selectorType === autoSelect) {
    // 1) Webhook-like contexts (Chat Trigger / Webhook) -> body.sessionId
    if ('getBodyData' in ctx) {
      const bodyData = ctx.getBodyData?.() ?? {};
      sessionId = (bodyData as Record<string, unknown>).sessionId as string;
    }

    // 2) Evaluate expression (defaults to {{ $json.sessionId }})
    if (!sessionId) {
      // If your node has a hidden string param `sessionIdExpression` with default
      // value `{{ $json.sessionId }}`, this lets users override it to things like:
      //   {{ $('When chat message received').first().json.sessionId }}
      const expr =
        (ctx.getNodeParameter(exprParamKey, itemIndex, '{{ $json.sessionId }}') as string) ??
        '{{ $json.sessionId }}';

      try {
        sessionId = ctx.evaluateExpression(expr, itemIndex) as string;
      } catch {
        // swallow and try legacy shim below
      }
    }

    // 3) (Optional) Legacy shim: call getChatTrigger() only if it exists at runtime
    if (!sessionId) {
      const anyCtx = ctx as unknown as { getChatTrigger?: () => { name?: string } | null };
      const chatTrigger = anyCtx.getChatTrigger?.();
      if (chatTrigger?.name) {
        try {
          ...
          // ignore – we'll error below if still empty
        }
      }
    }
    ...
    if (!sessionId) {
      throw new NodeOperationError(ctx.getNode(), 'No session ID found', {
        description:
          "Expected a 'sessionId' either on the incoming item ({{ $json.sessionId }}) or via your custom expression. To use something else, set the 'Session ID' parameter.",
        itemIndex,
      });
    }
  } else {
    ...
    if (!sessionId) {
      throw new NodeOperationError(ctx.getNode(), 'Key parameter is empty', {
        description:
          "Provide a key to use as session ID in the 'Key' parameter or switch to 'From input' to use the Chat Trigger session ID.",
        itemIndex,
      });
    }
  }
